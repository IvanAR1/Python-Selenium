...
def chargeEnv(file: str = "") -> dict:
    ...
    warn(
        "This method changed name to 'charge_env'.This function will be removed in a future.",
        DeprecationWarning,
    )
    charge_env(file)
...
def charge_env(file: str = "") -> None:
    ...
    file = str.format(file or ".env", project_name=(project, "")[project is None])
    if ExistFile(file):
        try:
            match extension_loader:
                case "env" | "":
                    if (
                        AcceptFile(file, f".{extension_loader}")
                        or file != f".{extension_loader}"
                    ):
    ...
    for key in keys:
        if isinstance(key, int):
            try:
                ...
                raise IndexError(f"Not exist key {key} in key {result}: f{e}")
